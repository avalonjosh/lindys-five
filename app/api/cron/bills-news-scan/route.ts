...
function generateTitle(topic: string) {
  const cleaned = stripCitationMarkup(topic).replace(/^the bills /i, 'Bills ').replace(/^bills /i, 'Bills ');
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // (topic feeds title, slug, metaDescription, storyKey, and sourceHeadlines).
