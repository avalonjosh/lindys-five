...
function formatNHLContext(schedule: any, roster: any, referenceDate: string) {
  ...
  playedGames.forEach((game: any) => {
    ...
    const wentToOT = game.gameOutcome?.lastPeriodType === 'OT' || game.gameOutcome?.lastPeriodType === 'SO';
...
async function fetchBillsData() {
  try {
    const standingsRes = await fetch('https://site.api.espn.com/apis/v2/sports/football/nfl/standings');
    const standings = await standingsRes.json();

    const rosterRes = await fetch('https://site.api.espn.com/apis/site/v2/sports/football/nfl/teams/2/roster');
    const roster = await rosterRes.json();
...
function formatBillsContext(standings: any, roster: any, referenceDate: string) {
  let billsRecord = 'N/A';
  let divisionPosition = 'N/A';

  try {
    const afcConf = standings.children?.find((conf: any) => conf.abbreviation === 'AFC');
    const afcEast = afcConf?.children?.find((div: any) => div.name === 'AFC East');
    const billsEntry = afcEast?.standings?.entries?.find(
      (entry: any) => entry.team?.abbreviation === 'BUF'
    );

    if (billsEntry) {
      const overallStat = billsEntry.stats?.find((s: any) => s.name === 'overall');
      billsRecord = overallStat?.displayValue || 'N/A';

      const position = afcEast?.standings?.entries?.findIndex(
        (e: any) => e.team?.abbreviation === 'BUF'
      );
      if (position !== -1) {
...
async function fetchSetData(setNumber: number) {
  const NHL_API_BASE = 'https://api-web.nhle.com/v1';
  ...
  try {
    ...
    const boxScorePromises = setGames.map((game: any) => fetchGameBoxScore(game.id));
    const boxScores = await Promise.all(boxScorePromises);

    return {
      setNumber,
      games: setGames,
      boxScores: boxScores.filter((b: any) => b !== null),
...
function formatBoxScore(boxscore: any, playByPlay: any, landing: any): string {
  ...
  let scoringSummary = '';
  const goals = (playByPlay?.plays || []).filter((p: any) => p.typeDescKey === 'goal');
  ...
  let penaltySummary = '';
  const penalties = (playByPlay?.plays || []).filter((p: any) => p.typeDescKey === 'penalty');
  penalties.slice(0, 10).forEach((pen: any) => {
...
function formatSetData(setData: any): string {
...
export async function POST(request: NextRequest) {
  ...
  const { idea, team, title, researchEnabled = false, allowedDomains, referenceDate, gameId, postType, setNumber } = await request.json();
  ...
  try {
    ...
    if (postType === 'game-recap' && gameId && team === 'sabres') {
      ...
      if (!boxData) {
        return NextResponse.json(
          { error: 'Failed to fetch game box score', details: 'Could not retrieve game data from NHL API' },
          { status: 500 }
        );
      }
...
The article should be 400-600 words and follow the style guidelines provided.`;
    ...
    if (postType === 'set-recap' && setNumber && team === 'sabres') {
      ...
      if (!setData || setData.boxScores.length === 0) {
        return NextResponse.json(
          { error: 'Failed to fetch set data', details: `Could not retrieve game data for set ${setNumber} from NHL API. The set may not be complete yet.` },
          { status: 500 }
        );
      }
      ...
      if (setData.boxScores.length < 3) {
        return NextResponse.json(
          { error: 'Insufficient game data', details: `Set ${setNumber} only has ${setData.boxScores.length} completed games. Need at least 3 games to generate a set recap.` },
          { status: 400 }
        );
      }
...
The article should be 400-800 words and follow the style guidelines provided.`;
    ...
    if (error.status === 401) {
      return NextResponse.json(
        { error: 'AI service authentication failed', details: 'Invalid API key. Please check ANTHROPIC_API_KEY in Vercel settings.' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to generate article', details: error.message || 'Unknown error', errorType: error.name || 'Error', status: error.status || null },
      { status: 500 }
    );
  }
}
