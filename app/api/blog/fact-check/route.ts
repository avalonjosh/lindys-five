...
// Fetch FULL game box score data
async function fetchFullGameBoxScore(gameId: string) {
  try {
...
function formatGameBoxScore(boxscore: any, playByPlay: any, landing: any) {
...
async function fetchSabresData() {
  try {
    ...
    if (roster.forwards) {
      roster.forwards.forEach((p: any) => rosterNames.push(`${p.firstName?.default} ${p.lastName?.default}`));
    }
    if (roster.defensemen) {
      roster.defensemen.forEach((p: any) => rosterNames.push(`${p.firstName?.default} ${p.lastName?.default}`));
    }
    if (roster.goalies) {
      roster.goalies.forEach((p: any) => rosterNames.push(`${p.firstName?.default} ${p.lastName?.default}`));
    }
    ...
    return {
      ...
      roster: rosterNames,
      recentGames: completedGames.slice(-10).map((g: any) => ({
        gameId: g.id,
        date: g.gameDate,
...
function formatTeamData(teamData: any): string {
  let dataBlock = `
...
`;
  ...
  if (teamData.recentGames?.length > 0) {
    dataBlock += `
RECENT GAMES:
${teamData.recentGames.map((g: any) => {
...
function formatFullGameData(gameData: any): string {
  return `
...
export async function POST(request: NextRequest) {
  ...
  const { content, team, type, gameId } = await request.json();
  ...
  try {
    ...
    if (!teamData) {
      return NextResponse.json(
        { error: 'Failed to fetch team data for fact checking', details: 'Could not retrieve data from official APIs' },
        { status: 500 }
      );
    }
...
Analyze the article and return your findings in JSON format. Focus ONLY on verifiable factual claims.`
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Failed to parse fact-check results', details: parseError.message, rawResponse: responseText },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to perform fact check', details: error.message },
      { status: 500 }
    );
  }
}
