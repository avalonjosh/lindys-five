...
export async function calculateChunkStats(chunk: GameChunk, teamId: number = 7): Promise<ChunkStats | null> {
  ...
  // Fetch detailed stats for each completed game SEQUENTIALLY to avoid rate limiting
  const gameStatsResults = [];
  for (const game of completedGames) {
    const stats = await fetchDetailedGameStats(game.gameId!, game.isHome, teamId);
    gameStatsResults.push(stats);
  }
